export const initUiSelect = (root = document) => {
    ...
    selects.forEach(select => {
        ...
        const hiddenInput = select.querySelector('input[type="hidden"][data-hidden]')
        ...
        const closeAllExcept = (except) => {
            document.querySelectorAll('[data-select].is-open').forEach(s => {
                if (s !== except) {
                    ...
                    const tr = s.querySelector('.ui-select__trigger')
...
export const setUiSelectDisabled = (selectEl, disabled, placeholderText, emptyText) => {
    ...
    const hidden = selectEl.querySelector('input[type="hidden"][data-hidden]')
...
export const renderOptions = (selectEl, items = [], emptyText = 'Нічого не знайдено') => {
    ...
    if (items.length) {
        list.insertAdjacentHTML(
            'afterbegin',
            items.map(i =>
                `<div class="ui-select__option" data-value="${i.id}" role="option" aria-selected="false">${i.name}</div>`
            ).join('')
        )
    }
